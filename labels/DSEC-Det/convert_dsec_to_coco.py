...
train_file = './test_2classes.csv'
class_list = './labels_filtered_map_2class.csv'

with open_for_csv(class_list) as file:
    classes = load_classes(csv.reader(file, delimiter=','))

with open_for_csv(train_file) as file:
...
for i in range(len(image_names)):
    ...
    image_info = {'file_name': file,
                'id': int(file.split('/')[-1].split('.npz')[0]),
                'height': 480,
                'width': 640}
    ...
    for ann in annot:
      ...
      ann = {'image_id': int(file.split('/')[-1].split('.npz')[0]),
